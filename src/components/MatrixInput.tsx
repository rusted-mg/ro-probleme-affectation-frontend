                ...
                <thead>
                    <tr>
                        ...
                        {matrix[0] && matrix[0].map((_, c) => (
                            <th key={c} className="border-2 border-blue-500 bg-blue-500 text-center px-2 py-1 font-medium text-white">
                                {c + 1}
                            </th>
                        ))}
                    </tr>
...
export const MatrixInput = ({ matrix, query, onMatrixChange }: MatrixInputProps) => {
    const solutionShowed = query.isSuccess && query.data?.job.status === "COMPLETED" && query.data?.job.result;
